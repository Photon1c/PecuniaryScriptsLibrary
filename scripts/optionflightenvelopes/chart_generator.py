"""" 
OPTION FLIGHT ENVELOPE CHART GENERATOR

requires data_loader module
...
def plot_flight_envelope_snapshot(
    ...
    for ticker, data in all_results.items():
        ...
        if np.isfinite(x) and np.isfinite(y_raw):
    ...
    info_text = []
    if raw_points:
        p = raw_points[0]  # Use first point for reference metrics
    ...
    # Add snapshot parameters below suptitle (like reference image)
    if raw_points:
        p = raw_points[0]
        params = []
