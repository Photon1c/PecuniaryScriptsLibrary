...
def plot_flight_envelope_snapshot(
    ...
    for ticker, data in all_results.items():
        ...
        # Get dealer pressure sign
        dps_val = dealer_pressure_data.get(ticker.upper()) if dealer_pressure_data else None
    ...
    for i, p in enumerate(points):
        ...
        # Build label based on tier
        if is_tier_c:
            label = None  # No label
        elif is_tier_a:
            # Tier A: Multi-line full label
            label_lines = [ticker]
            if is_multi_ticker and raw_p["x_nearest_wall_atr"] is not None and np.isfinite(raw_p["x_nearest_wall_atr"]):
                label_lines.append(f"w={raw_p['x_nearest_wall_atr']:.2f}ATR")
            if np.isfinite(rre_z):
                label_lines.append(f"rre={rre_z:+.2f}z")
            label_lines.append(f"DPS:{dps}")
            label = "\n".join(label_lines)
        else:  # Tier B
